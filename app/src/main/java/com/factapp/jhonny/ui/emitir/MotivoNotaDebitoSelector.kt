...
import com.factapp.jhonny.network.dto.MotivoNotaDebitoSunat
...
@Composable
fun MotivoNotaDebitoSelector(
    codigoSeleccionado: String?,
    onCodigoChange: (String) -> Unit,
    modifier: Modifier = Modifier,
) {
    var mostrarOpciones by remember { mutableStateOf(false) }
    val seleccionado = codigoSeleccionado?.let { MotivosNotaDebitoSunat.porCodigo(it) }

    Column(modifier = modifier.fillMaxWidth()) {
        Text(
            text = "Motivo de la nota de débito (SUNAT)",
            fontSize = 12.sp,
        ...
        Box(
            ...
            OutlinedTextField(
                ...
                placeholder = {
                    Text(
                        "Selecciona un motivo del catálogo 10",
        ...
        Text(
            text = "Catálogo N.° 10 — tipo de nota de débito electrónica",
            fontSize = 11.sp,
            color = C.textSecondary,
...
@Composable
private fun MotivoNotaDebitoOpcionesSheet(
    codigoSeleccionado: String?,
    onDismiss: () -> Unit,
    ...
    ModalBottomSheet(
        ...
        Column(modifier = Modifier.fillMaxWidth()) {
            EmitFormSheetHeader(
                titulo = "Motivo SUNAT",
                subtitulo = "Catálogo N.° 10 · ${MotivosNotaDebitoSunat.catalogo.size} opciones",
            ...
            LazyColumn(
                ...
                items(MotivosNotaDebitoSunat.comunes, key = { "comun-${it.codigo}" }) { motivo ->
                    MotivoNotaDebitoOpcionCard(
                        motivo = motivo,
                        seleccionado = motivo.codigo == codigoSeleccionado,
                        onClick = { onSeleccionar(motivo.codigo) },
                    )
                }
                if (MotivosNotaDebitoSunat.otros.isNotEmpty()) {
                    item {
                        Text(
                    ...
                    items(MotivosNotaDebitoSunat.otros, key = { "otro-${it.codigo}" }) { motivo ->
                        MotivoNotaDebitoOpcionCard(
                            motivo = motivo,
                            seleccionado = motivo.codigo == codigoSeleccionado,
                            onClick = { onSeleccionar(motivo.codigo) },
...
@Composable
private fun MotivoNotaDebitoOpcionCard(
    motivo: MotivoNotaDebitoSunat,
